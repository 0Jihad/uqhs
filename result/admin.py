...
from .models import ASUBJECTS, BTUTOR, CNAME, QSUBJECT, ANNUAL, TOTAL, Edit_User, RESULT_GRADE
...
class model_teacher(admin.ModelAdmin):
    list_display = ('model_in','user', 'cader', 'teacher_name', 'subject', 'Class', 'term', 'model_summary')
    fields = ['cader', 'model_in','user', 'teacher_name', 'subject', 'Class', 'term', 'graded', 'model_summary']
...
class model_totals(admin.ModelAdmin):
    list_display = ('model_in', 'subject_by', 'subject', 'subject_scores', 'subject_pert', 'term')
    fields = ['model_in', 'subject_by', 'subject', 'subject_scores', 'subject_pert', 'term']
@admin.register(ANNUAL)
class model_annual(admin.ModelAdmin):
    list_display = ('id', 'student_name', 'first', 'second', 'third', 'anual', 'agr', 'grade', 'anu_posi', 'subject_by', 'subject', 'term')
    fields = ['logged_in', 'student_name', ('first', 'second', 'third', 'anual'), ('agr', 'grade', 'anu_posi'), ('subject_by', 'subject'), 'term']
