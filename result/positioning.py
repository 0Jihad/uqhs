# -*- coding: utf-8 -*-
"""
Created on Fri Apr  6 15:42:45 2018

@author: JHD01831
"""
#mark collection as a list
import pandas as pd
def mark_list(dfg):
    marks_list = []
    for i in range(0, len(dfg)):
        marks_list.append(dfg.ix[i, -1])
    return marks_list
#Marks in the dataframe
#Determin mark positions by sorting
...
def marg_marks_positions(testlist, first_last_th, mark_orde):
    ...
    #results = pd.DataFrame(marg_list)
    #Transformed_results = results.T
    return marg_list
#from pos_sort import don_e
def don_e(df):
    testlist = df#mark_list(df)
    w = sort_pos(testlist)
    first_last_th = pos_t(w)
    results = marg_marks_positions(testlist, first_last_th, mark_orde)
    #results.columns = ['Mark', 'POS']
    #dr = pd.DataFrame(df)
    #dr['posi'] = results.POS
    return results
